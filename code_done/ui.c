...
static void on_encrypt_button_clicked(GtkWidget *widget, gpointer data)
{
    ...
    if (strlen(filename) == 0 || strlen(output_filename) == 0 || strlen(key) == 0)
    {
        g_print("Please select both input and output files, key.\n");
        return;
    }

    char file_name[MAX_FILENAME_LEN];
    char key_padded[KEY_LENGTH + 1];

    // Pad the key to 16 characters
    strncpy(key_padded, key, 16);
    size_t key_length = strlen(key);
    if (key_length < KEY_LENGTH)
    {
        memset(key_padded + key_length, ' ', KEY_LENGTH - key_length);
    }
    key_padded[KEY_LENGTH] = '\0'; // Ensure null-terminated string

    snprintf(file_name, MAX_FILENAME_LEN, "%s;%s;%s", filename, output_filename, key_padded);
...
int main(int argc, char *argv[])
{
    ...
    gtk_grid_attach(GTK_GRID(grid), label_key, 0, 2, 1, 1);
    entry_key = gtk_entry_new();
    gtk_grid_attach(GTK_GRID(grid), entry_key, 1, 2, 2, 1);
    gtk_entry_set_max_length(GTK_ENTRY(entry_key), KEY_LENGTH); // Set maximum length
    gtk_entry_set_width_chars(GTK_ENTRY(entry_key), 16);        // Set width in characters
    gtk_widget_set_hexpand(GTK_WIDGET(entry_key), TRUE);        // Expand horizontally
    gtk_widget_set_margin_end(GTK_WIDGET(entry_key), 10);
