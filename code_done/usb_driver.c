...
#include <linux/mutex.h> // struct device
#include "header.h"
...
unsigned char key1[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00};
...
static char file_input[MAX_FILENAME_LEN / 4] = {0};
static char file_output[MAX_FILENAME_LEN / 4] = {0};
static char input_key[BUF_SIZE + 1] = {0};
//
...
static int split_string(char *str, char *input, char *output, char *key, char delimiter)
{
    char *delim_pos1 = strchr(str, delimiter);
    if (!delim_pos1)
        return -EINVAL; // Delimiter not found, return error

    // Copy input part
    strncpy(input, str, delim_pos1 - str);
    input[delim_pos1 - str] = '\0';

    char *delim_pos2 = strchr(delim_pos1 + 1, delimiter);
    if (!delim_pos2)
        return -EINVAL;
    // Copy output part
    strncpy(output, delim_pos1 + 1, delim_pos2 - (delim_pos1 + 1));
    output[delim_pos2 - (delim_pos1 + 1)] = '\0';

    // Copy key part
    strcpy(key, delim_pos2 + 1);
    key[16] = '\0'; // Ensure null-terminated string

    return 0;
}
static long device_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    unsigned char *key_hex_buffer, *key_bytes;
    int retval = 0;
    int err = 0;
    int ret;
    key_hex_buffer = kmalloc(BUF_SIZE * 2 + 1, GFP_KERNEL);
    if (!key_hex_buffer)
    {
        printk(KERN_ALERT "Failed to allocate memory for key_hex_buffer\n");
        return -ENOMEM;
    }
    key_bytes = kmalloc(16, GFP_KERNEL);
    if (!key_bytes)
    {
        kfree(key_hex_buffer);
        printk(KERN_ALERT "Failed to allocate memory for key_bytesr\n");
        return -ENOMEM;
    }
    ...
    // ret = sscanf(file_name, "%[^;];%s", file_input, file_output);
    ret = split_string(file_name, file_input, file_output, input_key, ';');
    printk(KERN_INFO "File input: %s\n", file_input);
    printk(KERN_INFO "File output: %s\n", file_output);
    printk(KERN_INFO "File key: %s+abc\n", input_key);

    if (ret == 0)
    {
        printk(KERN_INFO "File input: %s\n", file_input);
        printk(KERN_INFO "File output: %s\n", file_output);
        printk(KERN_INFO "File key: %s\n", input_key);
    }
    ...
    {
        key_hex_buffer[w * 2] = (input_key[w] >> 4) + '0';
        if (key_hex_buffer[w * 2] > '9')
            key_hex_buffer[w * 2] += 7; // Chuyển số thành chữ cái A-F
        key_hex_buffer[w * 2 + 1] = (input_key[w] & 0xF) + '0';
        if (key_hex_buffer[w * 2 + 1] > '9')
            key_hex_buffer[w * 2 + 1] += 7; // Chuyển số thành chữ cái A-F
    }
    key_hex_buffer[2 * BUF_SIZE] = '\0';
    printk(KERN_INFO "Hex values: %s\n", key_hex_buffer);
    hex_string_to_bytes(key_hex_buffer, key_bytes);
    displayState(key_bytes);
    ExpandKey(key_bytes, expandedKey);
    printk(KERN_INFO "usb_crypto: Received filename from user-space: %s\n", file_name);
    ...
    case IOCTL_ENCRYPT:
        ...
        input_file = filp_open(file_input, O_RDONLY, 0);
        ...
        // Open output file
        output_file = filp_open(file_output, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    ...
    case IOCTL_DECRYPT:
        ...
        printk(KERN_INFO "File IO module loaded\n");
        // open input file
        inputcipher_file = filp_open(file_input, O_RDONLY, 0);
        ...
        // Open output file
        outputcipher_file = filp_open(file_output, O_WRONLY | O_CREAT | O_TRUNC, 0644);
