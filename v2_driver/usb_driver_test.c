...
static char file_input[BUFFER_SIZE] = {0};
static char file_output[BUFFER_SIZE] = {0};
static char stringKey[BUF_SIZE] = {0};
static char arrKey[BUF_SIZE] = {0};
unsigned char *hex_bufferKey;
...
{ // Điều khiển thiết bị (tạm thời không thực hiện gì)
    ...
    if (copy_from_user(&data, (struct ioctl_data *)ioctl_param, sizeof(struct ioctl_data)))
    {
        printk(KERN_ERR "Failed to copy data from user\n");
        return -EFAULT;
    }
    strncpy(file_input, data.input_filename, BUFFER_SIZE - 1);
    file_input[BUFFER_SIZE - 1] = '\0';
    strncpy(file_output, data.output_filename, BUFFER_SIZE - 1);
    file_output[BUFFER_SIZE - 1] = '\0';
    strncpy(stringKey, data.custom_string, BUF_SIZE - 1);
    stringKey[BUF_SIZE] = '\0';
    printk(KERN_INFO "usb_crypto: Received fileInput from user-space: %s\n", file_input);
    printk(KERN_INFO "usb_crypto: Received fileOutput from user-space: %s\n", file_output);
    printk(KERN_INFO "usb_crypto: Received fileKey from user-space: %s\n", data.custom_string);
    size_t len = strlen(data.custom_string);
    if (len == 0)
    {
        printk(KERN_INFO "usb_crypto: Key is empty\n");
        return 0;
    }
    else if (len < BUF_SIZE)
    {
        // Nếu chuỗi ngắn hơn MAX_STRING_LEN, thêm khoảng trắng ' ' để đủ MAX_STRING_LEN ký tự
        size_t l;
        for (l = len; l < BUF_SIZE; ++l)
        {
            stringKey[l] = ' ';
        }
        stringKey[BUF_SIZE] = '\0'; // Đảm bảo kết thúc chuỗi
    }
    printk(KERN_INFO "usb_crypto: Received Key from user-space: %s\n", stringKey);
    int n;
    for (n = 0; n < BUF_SIZE; ++n)
    {
        hex_bufferKey[n * 2] = (stringKey[n] >> 4) + '0';
        if (hex_bufferKey[n * 2] > '9')
            hex_bufferKey[n * 2] += 7; // Chuyển số thành chữ cái A-F
        hex_bufferKey[n * 2 + 1] = (stringKey[n] & 0xF) + '0';
        if (hex_bufferKey[n * 2 + 1] > '9')
            hex_bufferKey[n * 2 + 1] += 7; // Chuyển số thành chữ cái A-F
    }
    hex_bufferKey[2 * BUF_SIZE] = '\0'; // Đảm bảo chuỗi kết thúc bằng null-terminator
    ...
    hex_string_to_bytes(hex_bufferKey, bytesKey);
    //   bytes là con trỏ mảng
    displayState(bytesKey);
    ExpandKey(bytesKey, expandedKey);
